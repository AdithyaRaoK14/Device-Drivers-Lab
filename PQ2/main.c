...
long scull_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
	  ...
	  case SORT_NTH_QUANTUM: 
	 	struct scull_dev *d = &scull_devices[0]; /* first ScullDev */
	  	int nth_quantum;
	  	char *q;
	  	int len;
	  	char temp;
	  	...
	  	if (__get_user(nth_quantum, (int __user *)arg))
	  		return -EFAULT;

	  	if (!d || !d->data || !d->data->data || !d->data->data[nth_quantum] || nth_quantum >= d->qset)
	  		break;

	  	q   = (char *)d->data->data[nth_quantum]; /* nth quantum */
	  	
	  	len = d->size - (long)nth_quantum * d->quantum;
		if (len > d->quantum)
    		len = d->quantum;
