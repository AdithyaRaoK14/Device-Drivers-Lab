...
//cr s
/* The scullsort proc implementation.*/

/* Write handler: echo "device_index" > /proc/scullsort
 * Sorts characters in first quantum of that device.
 * Example: echo "0" > /proc/scullsort  → sorts quantum[0] of scull0 */
static ssize_t scull_write_sort(struct file *filp, const char __user *buf,
                                size_t count, loff_t *ppos)
{
    ...
    if (sscanf(kbuf, "%d", &dev_index) != 1)
        return -EINVAL;

    if (dev_index < 0 || dev_index >= scull_nr_devs)
        return -EINVAL;

    d = &scull_devices[dev_index];

    if (mutex_lock_interruptible(&d->mutex))
        return -ERESTARTSYS;

    /* check first quantum exists */
    if (!d->data || !d->data->data || !d->data->data[0]) {
        mutex_unlock(&d->mutex);
        return -ENODATA;
    }

    q   = (char *)d->data->data[0];
    len = min_t(int, d->quantum, (int)d->size);
    ...
    printk(KERN_INFO "scull%d: first quantum sorted (%d bytes)\n",
           dev_index, len);
    return count;
}

/* Read handler: shows first 50 chars of first quantum for all devices */
static int scull_read_sort(struct seq_file *s, void *v)
{
    int i, j;
    char safe_buf[51];
    int print_len;
    ...
    for (i = 0; i < scull_nr_devs; i++) {
        ...
        if (!d->data || !d->data->data || !d->data->data[0] || d->size == 0) {
            seq_printf(s, "scull%d: (empty)\n", i);
            mutex_unlock(&d->mutex);
            continue;
        }

        print_len = min_t(int, 50, min_t(int, d->quantum, (int)d->size));
        memset(safe_buf, 0, sizeof(safe_buf));

        for (j = 0; j < print_len; j++) {
            unsigned char c = ((unsigned char *)d->data->data[0])[j];
            safe_buf[j] = (c >= 32 && c < 127) ? c : '.';
        }

        seq_printf(s, "scull%d: first quantum (first %d chars): %s\n",
                   i, print_len, safe_buf);
...
static int scull_sort_proc_open(struct inode *inode, struct file *filp)
{
    return single_open(filp, scull_read_sort, NULL);
}

static struct proc_ops scullsort_proc_ops = {
    .proc_open    = scull_sort_proc_open,
    .proc_read    = seq_read,
    .proc_write   = scull_write_sort,
    .proc_lseek   = seq_lseek,
    .proc_release = single_release,
};
...
static void scull_create_proc(void)
{
	proc_create_data("scullmem", 0 /* default mode */,
			...
			NULL /* client data */);
    proc_create("scullsort", 0666, NULL, &scullsort_proc_ops); //cr
}
...
static void scull_remove_proc(void)
{
	...
	remove_proc_entry("scullsort", NULL); //cr
}
