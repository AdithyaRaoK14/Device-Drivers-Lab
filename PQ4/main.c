...
static int scull_read_procmem(struct seq_file *s, void *v)
{
    ...
    for (i = 0; i < scull_nr_devs && s->count <= limit; i++) {
        ...
        struct scull_qset *qs = d->data;
        int print_len, j;
        char safe_buf[51]; /* 50 chars + null terminator */
        ...
        /* Print first 50 chars of first quantum if data exists */
        if (qs && qs->data && qs->data[0]) {
            print_len = min(50, d->quantum);
            print_len = min_t(int, print_len, (int)d->size);

            memset(safe_buf, 0, sizeof(safe_buf));
            for (j = 0; j < print_len; j++) {
                unsigned char c = ((unsigned char *)qs->data[0])[j];
                safe_buf[j] = c;
            }
            seq_printf(s, "  first quantum (first %d bytes): %s\n",
                    print_len, safe_buf);
        } else {
            seq_printf(s, "  first quantum: (empty)\n");
        }
