...
//cr s
/* The scullquantum proc implementation.*/

/* Write handler: echo "device_index new_quantum" > /proc/scullquantum
 * Example: echo "0 8000" > /proc/scullquantum  → sets scull0 quantum to 8000 */
static ssize_t scull_write_quantum(struct file *filp, const char __user *buf,
                                   size_t count, loff_t *ppos)
{
    char kbuf[32];
    int dev_index, new_quantum;
    ...
    /* parse "device_index new_quantum" */
    if (sscanf(kbuf, "%d %d", &dev_index, &new_quantum) != 2)
        return -EINVAL;
    ...
    if (mutex_lock_interruptible(&scull_devices[dev_index].mutex))
        return -ERESTARTSYS;

    scull_devices[dev_index].quantum = new_quantum;

    mutex_unlock(&scull_devices[dev_index].mutex);

    printk(KERN_INFO "scull%d quantum changed to %d\n", dev_index, new_quantum);
    return count;
}

/* Read handler: shows current quantum of all devices */
static int scull_read_quantum(struct seq_file *s, void *v)
{
    int i;
    for (i = 0; i < scull_nr_devs; i++) {
        if (mutex_lock_interruptible(&scull_devices[i].mutex))
            return -ERESTARTSYS;
        seq_printf(s, "scull%d: quantum = %d\n", i, scull_devices[i].quantum);
        mutex_unlock(&scull_devices[i].mutex);
    }
    return 0;
}

static int scull_quantum_proc_open(struct inode *inode, struct file *filp)
{
    return single_open(filp, scull_read_quantum, NULL);
}

static struct proc_ops scullquantum_proc_ops = {
    .proc_open    = scull_quantum_proc_open,
    .proc_read    = seq_read,
    .proc_write   = scull_write_quantum,
    .proc_lseek   = seq_lseek,
    .proc_release = single_release,
};
//cr 
...
static void scull_create_proc(void)
{
	proc_create_data("scullmem", 0 /* default mode */,
			...
			NULL /* client data */);
    proc_create("scullquantum", 0666, NULL, &scullquantum_proc_ops); //cr
}
...
static void scull_remove_proc(void)
{
	...
	remove_proc_entry("scullquantum", NULL); //cr
}
