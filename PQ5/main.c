...
static int scull_read_procmem(struct seq_file *s, void *v)
{
    ...
    for (i = 0; i < scull_nr_devs && s->count <= limit; i++) {
        ...
        if (d->size == 0 || !d->data) {
            seq_printf(s, "  last 50 chars: (empty)\n");
            mutex_unlock(&d->mutex);
            continue;
        }

        /* how many bytes to print — up to 50, capped at actual size */
        print_len = (d->size < 50) ? (int)d->size : 50;
        start_pos = d->size - print_len;
        itemsize  = d->quantum * d->qset;

        memset(safe_buf, 0, sizeof(safe_buf));
        j = 0;

        while (j < print_len) {
            loff_t cur_pos = start_pos + j;
            unsigned char c;

            /* locate qset node, quantum index, offset within quantum */
            item  = (long)cur_pos / itemsize;
            rest  = (long)cur_pos % itemsize;
            s_pos = rest / d->quantum;
            q_pos = rest % d->quantum;

            /* walk to correct qset node */
            qs = d->data;
            while (item-- && qs)
                qs = qs->next;
            ...
            c = ((unsigned char *)qs->data[s_pos])[q_pos];
            safe_buf[j] = c;
            j++;
        }

        seq_printf(s, "  last %d chars: %s\n", print_len, safe_buf);
        mutex_unlock(&d->mutex);
    }
    return 0;
}
