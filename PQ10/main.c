...
//cr s
/* The scullblank proc implementation.*/

/* Write handler: echo "device_index" > /proc/scullblank
 * Blanks (zeroes) the first quantum of that device.
 * Example: echo "0" > /proc/scullblank */
static ssize_t scull_write_blank(struct file *filp, const char __user *buf,
                                 size_t count, loff_t *ppos)
{
    ...
    /* zero out entire first quantum buffer */
    memset(d->data->data[0], 0, d->quantum);

    mutex_unlock(&d->mutex);

    printk(KERN_INFO "scull%d: first quantum blanked (%d bytes zeroed)\n",
           dev_index, d->quantum);
    return count;
}

/* Read handler: shows first 50 chars of first quantum for all devices */
static int scull_read_blank(struct seq_file *s, void *v)
{
    int i, j;
    char safe_buf[51];
    int print_len;
...
static int scull_blank_proc_open(struct inode *inode, struct file *filp)
{
    return single_open(filp, scull_read_blank, NULL);
}

static struct proc_ops scullblank_proc_ops = {
    .proc_open    = scull_blank_proc_open,
    .proc_read    = seq_read,
    .proc_write   = scull_write_blank,
    .proc_lseek   = seq_lseek,
    .proc_release = single_release,
};
...
static void scull_create_proc(void)
{
	proc_create_data("scullmem", 0 /* default mode */,
			...
			NULL /* client data */);
    proc_create("scullblank", 0666, NULL, &scullblank_proc_ops); //cr
}
...
static void scull_remove_proc(void)
{
	...
	remove_proc_entry("scullblank", NULL); //cr
}
